...
const Banner = function ({}: Props) {
    ...
    return (
        <div>
            <Image
                className="absolute content-[''] top-0 left-0 bottom-0 right-0 w-full h-full bg-[rgba(0,0,0.3)] backdrop-blur-3xl blur-[15px]"
                src={image}
                alt="Background"
            />
            <div className="relative px-4 my-0 mx-auto w-full max-w-[1200px] max-sm:px-12 max-md:px-20 bg-no-repeat bg-cover overflow-hidden transition-all duration-75 ease-in-out">
                        ...
                        {banners.map(function (banner: BannerType, index: number) {
                            if (banner.image === image) {
                                return (
                                    ...
                                    >
                                        <div className="absolute z-100 top-0 left-0 opacity-100 bg-white w-full h-full">
                                            <h2>{banner.button}</h2>
