...
const Title = function ({ title, description, className }: Props) {
    return (
        <header
            className={`flex items-center justify-center flex-col  mb-7 relative px-4 my-0 mx-auto w-full max-w-[1200px] max-sm:px-12 max-md:px-20 ${className}`}
        >
            <div className="relative w-fit">
                <h3 className="text-[#EE4222] font-semibold  text-[40px] text-center mb-[10px] mt-8 relative">
                    {title}
                </h3>
                <svg
                    className="absolute bottom-[-8px] w-16 right-[-24px]"
            ...
            {description && (
                <p
                    className={
                        " text-center text-[20px] font-normal leading-normal text-[#EE4222] max-w-[600px]"
                    }
                >
                    {description}
                </p>
